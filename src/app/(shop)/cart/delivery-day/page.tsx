...
import { DeliveryDatesService, DeliveryOption } from "@/services/settings/orders"
import { getAddresses } from "@/services/settings/address"
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (defaultAddress) {
          ...
          const options = await DeliveryDatesService.getDeliveryOptions(defaultAddress)
        ...
        <div className="flex flex-col lg:flex-row justify-start items-start gap-8">
          ...
          <div className="w-full lg:w-[847px] space-y-8">
            ...
            <div className="p-6 bg-gray rounded-lg space-y-6">
              <Subheading className="text-white">Opciones de entrega</Subheading>
              <div className="h-px bg-white/20"></div>
              
              {loading ? (
                <div className="text-center py-8">
                  <Body className="text-white">Cargando opciones de entrega...</Body>
                </div>
              ) : deliveryOptions.length === 0 ? (
                <div className="text-center py-8">
                  <Body className="text-white">No hay opciones de entrega disponibles</Body>
                </div>
              ) : (
                  ...
                  {deliveryOptions.map((option) => (
                    ...
                    >
                      <div className={`w-5 h-5 p-2.5 rounded-[99px] border border-cyan-400 flex justify-center items-center ${
                        selectedDeliveryOption === option.id ? "bg-cyan-400" : ""
                      }`}>
                      ...
                      <div className="flex-1 flex justify-between items-center">
                        <div className="flex flex-col gap-1">
                          ...
                          <Body className="text-zinc-400 text-sm">{option.description}</Body>
                        </div>
                        <Body className={`text-xl ${option.cost === 0 ? 'text-emerald-400' : 'text-white'}`}>
          ...
          <div className="flex-1 w-full lg:w-auto">
            <div className="p-6 bg-gray rounded-lg space-y-6">
              ...
              <div className="flex justify-between items-center">
                ...
                <Body className="text-white text-xl">${shippingCost.toLocaleString()}</Body>
              </div>
              <div className="h-px bg-white/20"></div>
              <div className="flex justify-between items-center">
