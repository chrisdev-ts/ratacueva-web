...
import { getAddresses, Address } from "@/services/settings/address"
...
function FormPageContent() {
  ...
  const handleContinue = () => {
    ...
    const shippingData = {
      ...
      shippingAddress: selectedDeliveryOption === "domicilio" && selectedAddress ? {
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-start gap-3">
                      <div className="flex-1">
                        <Body className="text-white">
                          {selectedAddress ? formatAddress(selectedAddress) : "Selecciona una dirección"}
                        </Body>
                      </div>
                    </div>
                    ...
                    {addresses.length > 1 && (
                      <div className="space-y-2">
                        <Body className="text-white text-sm">Seleccionar dirección:</Body>
                        {addresses.map((address) => (
                          ...
                          >
                            <Body className="text-white text-sm">
                              ...
                              {address.isDefault && (
                                <span className="ml-2 text-cyan-400 text-xs">(Predeterminada)</span>
                              )}
                            </Body>
                          </div>
                        ))}
          ...
          <div className="flex-1 w-full lg:w-auto">
            <div className="p-6 bg-gray rounded-lg space-y-6">
              ...
              <div className="flex justify-between items-center">
                ...
                <Body className="text-white">${selectedDeliveryOption === "domicilio" ? shippingCost.toLocaleString() : "0"}</Body>
              </div>
              <div className="h-px bg-white/20"></div>
              <div className="flex justify-between items-center">
                <Body className="text-white">Total</Body>
                <Subheading className="text-white">${(subtotal + (selectedDeliveryOption === "domicilio" ? shippingCost : 0)).toLocaleString()}</Subheading>
              </div>
            </div>
          </div>
        </div>
